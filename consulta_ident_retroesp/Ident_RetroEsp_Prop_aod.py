# Identificación RetroEsp
#Territorio cedido -> cPAI_Pais (cTERRIN_Id) para traer cTER_Id
#Nombres cedente (nuevo cruce)
#%% Librerías
...
#%% Selección del driver ODBC más estable disponible
# El driver antiguo "{SQL Server}" usa DBNETLIB, que corta la conexión en consultas
# largas (errores 10054 / "Error general de red"). Si la máquina tiene instalado un
# driver moderno (ODBC Driver 17/18), se usa ese: es mucho más estable.
def _elige_driver():
    try:
...
driver = _elige_driver()

# Crea la cadena de conexión
conn_str = f"DRIVER={{{driver}}};SERVER={server};DATABASE={database};UID={username};PWD={password}"
# Los drivers modernos exigen definir el cifrado; se confía en el certificado del servidor.
if driver.startswith('ODBC Driver'):
    conn_str += ";Encrypt=no;TrustServerCertificate=yes"

print(f"Driver ODBC en uso: {driver}")

#%% Lectura robusta de consultas (reintentos + reconexión + lectura por partes)
# La red/servidor puede cortar la conexión al traer resultados de tablas grandes
# (error 10054 / DBNETLIB). Esta función reabre la conexión y reintenta,
# y trae los datos por bloques (chunks) para no saturar la transmisión.
def leer_sql(sql, etiqueta="consulta", intentos=3, espera=10, chunksize=50000):
    ultimo_error = None
    for intento in range(1, intentos + 1):
        conn_local = None
        try:
            _t0 = time.perf_counter()
            conn_local = pyodbc.connect(conn_str)
            partes = []
            for bloque in pd.read_sql(sql, conn_local, chunksize=chunksize):
                partes.append(bloque)
            conn_local.close()
            df = pd.concat(partes, ignore_index=True) if partes else pd.DataFrame()
            _t1 = time.perf_counter()
            print(f"  [{etiqueta}] OK — {len(df):,} filas en {_t1 - _t0:,.1f} s")
            return df
            ...
            if conn_local is not None:
                try:
                    ...
                    pass
            if intento < intentos:
                print(f"  [{etiqueta}] intento {intento}/{intentos} falló ({type(e).__name__}). Reintentando en {espera}s...")
                time.sleep(espera)
            else:
                print(f"  [{etiqueta}] falló tras {intentos} intentos.")
    raise ultimo_error

# Establece la conexión
# (la conexión la abre y cierra leer_sql en cada consulta)


#%% Cedido Facultativo
#Funciones con STUFF acumula los valores en un solo string separado con comas de una subconsulta)

Cedido = f"SELECT T1.cCOR_Id as Corredor, T1.aHCP_NumContrato as NoContrato, T1.aHCP_IdentContrato as Ident_Contrato, T1.aHCP_Vig as Año_Vigencia, T1.aHCP_FecRenovacion as Inicio_Vigencia, T1.aHCP_FecFinVigencia as Fin_Vigencia, T1.aHCP_Nombre as Nombre, T1.aHCP_NombreIng as NombreIng, T18.cTRE_Id as DescComision, T18.rSobCom_PorcSobCom as ComPrimB, T18.rSobCom_PorcSobComPrimNeta as ComPrimNeta, " \
    f"STUFF((SELECT DISTINCT ', ' + CAST(T0.cRAM_Id AS varchar(50)) FROM rRamRegXHCP T0 WHERE T1.cCOR_Id=T0.cCOR_Id AND T1.cCOR_Version=T0.cCOR_Version AND T1.aHCP_NumContrato=T0.aHCP_NumContrato AND T1.aHCP_IdentContrato=T0.aHCP_IdentContrato AND T1.aHCP_Vig=T0.aHCP_Vig AND T1.aHCP_Estatus=T0.aHCP_Estatus FOR XML PATH ('')), 1, 2, '') AS Ramos_Cubiertos, " \
    f"STUFF((SELECT DISTINCT ', ' + CAST(T0.cSBP_Id AS varchar(50)) FROM rSubExcXHCP T0 WHERE T1.cCOR_Id=T0.cCOR_Id AND T1.cCOR_Version=T0.cCOR_Version AND T1.aHCP_NumContrato=T0.aHCP_NumContrato AND T1.aHCP_IdentContrato=T0.aHCP_IdentContrato AND T1.aHCP_Vig=T0.aHCP_Vig AND T1.aHCP_Estatus=T0.aHCP_Estatus FOR XML PATH ('')), 1, 2, '') AS Subramos_Excluidos, " \
    f"STUFF((SELECT DISTINCT ', ' + CAST(T0.cTER_Id AS varchar(50)) FROM rTerrCubXHCP T0 WHERE T1.cCOR_Id=T0.cCOR_Id AND T1.cCOR_Version=T0.cCOR_Version AND T1.aHCP_NumContrato=T0.aHCP_NumContrato AND T1.aHCP_IdentContrato=T0.aHCP_IdentContrato AND T1.aHCP_Vig=T0.aHCP_Vig AND T1.aHCP_Estatus=T0.aHCP_Estatus FOR XML PATH ('')), 1, 2, '') AS Territorios_Cubiertos, " \
    f"STUFF((SELECT DISTINCT ', ' + CAST(T0.cPAI_Id AS varchar(50)) FROM rPaiExcXHCP T0 WHERE T1.cCOR_Id=T0.cCOR_Id AND T1.cCOR_Version=T0.cCOR_Version AND T1.aHCP_NumContrato=T0.aHCP_NumContrato AND T1.aHCP_IdentContrato=T0.aHCP_IdentContrato AND T1.aHCP_Vig=T0.aHCP_Vig AND T1.aHCP_Estatus=T0.aHCP_Estatus FOR XML PATH ('')), 1, 2, '') AS Paises_Excluidos, " \

# Contratos del tomado correspondientes a los contratos del cedido
Tomado = f"COALESCE(" \
...
f"T6.aOFD_NoContr as _k_ofd_nocontr, T6.cTER_Id as _k_ofd_cter, T6.aOFD_AnioVig as _k_ofd_anio, T6.aOFD_NoOferta as _k_ofd_oferta, T6.aOFD_NoEndoso as _k_ofd_endoso, " \
...
f"T4.cCOR_Id as _k_htp_cor, T4.cCIA_Id as _k_htp_cia, T4.aHTP_NoContr as _k_htp_nocontr, T4.aHTP_AnioVig as _k_htp_anio " \

#Contratos del cedido -> Cuadro Retro/Cuadro Retro Ofertas -> Hojas del tomado (Prop, NoProp - Tabla capas por renglón, No prop - Tabla capas por capa, Fac Daños, Fac Fianzas, Fac Vida ind, Fac Vida gpo)
#No proporcional realiza dos subconsulta con la tabla de las capas, una cruza utilizando la llave + renglón y otro con la llave + capa
#Últimos cuatro LEFT JOIN son para obtener las referencias (descripción de los contratos) del tomado facultativo
xFROM = f" FROM pDBSirec2.dbo.aHCP_CedProporcional as T1 " \
            f" LEFT JOIN rCRHD_CuadroRetroHD as T2 on T1.cCOR_Id=T2.cCOR_IdCed AND T1.cCOR_Version=T2.cCOR_VersionCed AND T1.aHCP_NumContrato=T2.aHC_NumContrato AND T1.aHCP_IdentContrato=T2.aHC_IdentContrato AND T1.aHCP_Vig=T2.aHC_Vig " \
            f" LEFT JOIN rCROF_CuadroRetroOF as T3 on T1.cCOR_Id=T3.cCOR_IdCed AND T1.cCOR_Version=T3.cCOR_VersionCed AND T1.aHCP_NumContrato=T3.aHC_NumContrato AND T1.aHCP_IdentContrato=T3.aHC_IdentContrato AND T1.aHCP_Vig=T3.aHC_Vig " \
            ...
            f" LEFT JOIN rSobComXHCP as T18 on T1.cCOR_Id=T18.cCOR_Id AND T1.cCOR_Version=T18.cCOR_Version AND T1.aHCP_NumContrato=T18.aHCP_NumContrato AND T1.aHCP_IdentContrato=T18.aHCP_IdentContrato AND T1.aHCP_Vig=T18.aHCP_Vig  "
#xWHERE = f" WHERE T1.aHCP_Estatus = 1 AND T1.aHCP_Vig >= 2024 " 
xWHERE = f" WHERE T1.aHCP_Estatus = 1 AND T1.aHCP_FecInicio > 45292" 
#xWHERE = f" WHERE T1.aHCP_Estatus = 1 AND T1.aHCP_FecInicio >= 45292 " 
xORDERBY = f" ORDER BY T1.aHCP_NumContrato "

xSQL = " ".join([Cedido, Tomado, xFROM, xWHERE, xORDERBY])

CedF = leer_sql(xSQL, etiqueta='Consulta principal (cedido/tomado)')

#%% Campos pesados traídos con consultas ligeras + agregación/concatenación en pandas
# (Comisión Original, Impuestos rHTP, Componentes SA, IVA/ISR de Daños).
# Se traen las tablas con SELECT simples y se cruzan en pandas por las llaves auxiliares
# expuestas en CedF (_k_htp_* para Tomado Prop, _k_ofd_* para Oferta Daños).
# Esto evita las subconsultas correlacionadas que hacían lentísima la consulta principal.

# 1) Comisión Original (rPCOXHTP) -> SUM por contrato del Tomado Prop
_sql_comision = ' SELECT cCOR_Id, cCIA_Id, aHTP_NoContr, aHTP_AnioVig, ' \
                ' SUM(rPCO_MntComision) as Comision_Original_TP ' \
                ' FROM pDBSirec2.dbo.rPCOXHTP ' \
                ' GROUP BY cCOR_Id, cCIA_Id, aHTP_NoContr, aHTP_AnioVig '
Comision = leer_sql(_sql_comision, etiqueta='Comisión original')

# 2) Impuestos (rHTP_ImpuestosPaisHP) -> SUM por contrato del Tomado Prop
_sql_impuestos = ' SELECT cCor_Id, cCia_Id, rHTP_NoContr, rHTP_AnioVig, ' \
                 ' SUM(rHTP_ISR) as ISR_TP, SUM(rHTP_Interes) as Interes_TP, ' \
                 ' SUM(rHTP_Impuestos) as Impuestos_TP, SUM(rHTP_ImpuestosSInteres) as Impuestos_sInteres_TP ' \
                 ' FROM pDBSirec2.dbo.rHTP_ImpuestosPaisHP ' \
                 ' GROUP BY cCor_Id, cCia_Id, rHTP_NoContr, rHTP_AnioVig '
Impuestos = leer_sql(_sql_impuestos, etiqueta='Impuestos (rHTP)')

# 3) Componentes de Suma Asegurada (rCECXOFD) -> se concatenan en pandas por oferta
_sql_componentes = ' SELECT aOFD_NoContr, cTER_Id, aOFD_AnioVig, aOFD_NoOferta, aOFD_NoEndoso, ' \
                   ' rCEC_Nombre, rCEC_MntPrc ' \
                   ' FROM pDBSirec2.dbo.rCECXOFD '
Componentes = leer_sql(_sql_componentes, etiqueta='Componentes SA')

# 4) IVA s/Corretaje e ISR de Daños (aOFD_OfertaDCompl) -> 1 por oferta
_sql_ivaisr = ' SELECT aOFD_NoContr, cTER_Id, aOFD_AnioVig, aOFD_NoOferta, aOFD_NoEndoso, ' \
              ' aOFD_IVASinCorretaje, aOFD_ISR ' \
              ' FROM pDBSirec2.dbo.aOFD_OfertaDCompl '
IvaIsr = leer_sql(_sql_ivaisr, etiqueta='IVA/ISR Daños')

# Cierra la conexión
# (conexión cerrada por leer_sql)

# ---- Procesamiento en pandas ----
# Llaves de cruce
_llaves_htp = ['_k_htp_cor', '_k_htp_cia', '_k_htp_nocontr', '_k_htp_anio']
_llaves_ofd = ['_k_ofd_nocontr', '_k_ofd_cter', '_k_ofd_anio', '_k_ofd_oferta', '_k_ofd_endoso']

# 1) Merge Comisión (por llaves del Tomado Prop)
Comision = Comision.rename(columns={'cCOR_Id':'_k_htp_cor', 'cCIA_Id':'_k_htp_cia',
                                    'aHTP_NoContr':'_k_htp_nocontr', 'aHTP_AnioVig':'_k_htp_anio'})
CedF = CedF.merge(Comision, how='left', on=_llaves_htp)

# 2) Merge Impuestos (por llaves del Tomado Prop)
Impuestos = Impuestos.rename(columns={'cCor_Id':'_k_htp_cor', 'cCia_Id':'_k_htp_cia',
                                      'rHTP_NoContr':'_k_htp_nocontr', 'rHTP_AnioVig':'_k_htp_anio'})
CedF = CedF.merge(Impuestos, how='left', on=_llaves_htp)

# 3) Componentes SA: concatenar "Nombre: Monto" por oferta y merge (por llaves de Oferta Daños)
def _concat_componentes(grupo):
    partes = []
    for _, r in grupo.iterrows():
        nombre = '' if pd.isna(r['rCEC_Nombre']) else str(r['rCEC_Nombre'])
        monto = '' if pd.isna(r['rCEC_MntPrc']) else str(r['rCEC_MntPrc'])
        partes.append(f"{nombre}: {monto}")
    return ', '.join(partes)

if len(Componentes) > 0:
    Componentes = Componentes.rename(columns={'aOFD_NoContr':'_k_ofd_nocontr', 'cTER_Id':'_k_ofd_cter',
                                              'aOFD_AnioVig':'_k_ofd_anio', 'aOFD_NoOferta':'_k_ofd_oferta',
                                              'aOFD_NoEndoso':'_k_ofd_endoso'})
    Componentes_agg = Componentes.groupby(_llaves_ofd).apply(_concat_componentes).reset_index(name='SA_Componentes_OFD')
    CedF = CedF.merge(Componentes_agg, how='left', on=_llaves_ofd)
else:
    CedF['SA_Componentes_OFD'] = None

# 4) IVA/ISR de Daños: merge directo (por llaves de Oferta Daños)
IvaIsr = IvaIsr.rename(columns={'aOFD_NoContr':'_k_ofd_nocontr', 'cTER_Id':'_k_ofd_cter',
                                'aOFD_AnioVig':'_k_ofd_anio', 'aOFD_NoOferta':'_k_ofd_oferta',
                                'aOFD_NoEndoso':'_k_ofd_endoso',
                                'aOFD_IVASinCorretaje':'IVA_sCorretaje_OFD', 'aOFD_ISR':'ISR_OFD'})
# Puede haber más de una fila por oferta en la complementaria: quedarse con la primera
IvaIsr = IvaIsr.drop_duplicates(subset=_llaves_ofd, keep='first')
CedF = CedF.merge(IvaIsr, how='left', on=_llaves_ofd)

# Eliminar las llaves auxiliares (ya no se necesitan; no van en el reporte)
CedF = CedF.drop(columns=_llaves_htp + _llaves_ofd)
...
MovTom = leer_sql(xSQL, etiqueta='Movimientos contables')
...
MonPol = leer_sql(xSQL_MonPol, etiqueta='Moneda y póliza')
...
#MovTom['LLAVE'] = MovTom[['cCOR_IdOrig', 'cCIA_IdOrig', 'aMOV_IdentContrOrig', 'aMOV_AñoOrig']].apply(lambda x: '-'.join(x.astype('str')), axis=1)
#CedF['LLAVE'] = CedF[['Corredor_', 'NoCedente', 'NoContrato_', 'Año_Vigencia_']].apply(lambda x: '-'.join(x.astype('str')), axis=1)
#CedF['LLAVE'] = CedF[['Corredor_', 'NoCedente', 'NoContrato_', 'Año_Vigencia_']].apply(lambda x: '-'.join(x.astype(int).astype(str)), axis=1)
...
#CedF = CedF.merge(MovTom[["LLAVE", "PrimaNal", "ComisionesNal", "SiniestrosNal"]].drop_duplicates(),
...
def to_set(value):
    ...
    Maneja: NaN, None, números sueltos, strings sin espacios, strings con espacios, listas.

    CORRECCIÓN: antes, un valor numérico legítimo (p. ej. un único ramo/subramo que
    llega como 60.0 en lugar de '60') se descartaba por completo (isinstance(value,
    float) -> set() vacío). Con el conjunto vacío, las validaciones regresaban
    VERDADERO por omisión aunque sí existiera un traslape real que debía marcarse
    FALSO. Ahora el número se conserva como texto sin decimales ('60'), y además se
    normalizan elementos tipo '60.0' -> '60' para que comparen igual contra '60'.
    """
    if pd.isna(value):
        return set()
...
def _a_fecha(valor):
    """
    Normaliza cualquier representación de fecha a un Timestamp sin hora.
    Acepta datetime/Timestamp/date, texto y seriales numéricos tipo Excel
    (días desde 1899-12-30, como aHCP_FecInicio). Regresa None si no hay
    fecha válida. Esto evita comparaciones entre tipos mezclados (que pueden
    aparecer cuando la consulta se lee por bloques) y errores por hora del día.
    """
    if pd.isna(valor):
        return None
    ...
    return None if pd.isna(f) else pd.Timestamp(f).normalize()

def comparar_vigencias(row):
    fecha_inicio_cedido = _a_fecha(row['Inicio_Vigencia'])
    fecha_inicio_tomado = _a_fecha(row['Inicio_Vigencia_'])

    # CORRECCIÓN: cuando la fila del cedido no tiene contrato tomado asociado
    # (o falta la fecha en alguno de los dos lados), no hay nada que validar.
    # Antes, la comparación NaT >= fecha regresaba False y la fila se marcaba
    # FALSO en 'Validacion Vigencia' aunque las otras cuatro validaciones
    # (ramos, subramos, territorios, países) sí regresan VERDADERO en ese mismo
    # caso de tomado vacío. Ahora las cinco validaciones son consistentes.
    if fecha_inicio_tomado is None or fecha_inicio_cedido is None:
        return True
...
#Funcion Validación Ramos: Valida si los ramos del tomado se encuentran en los ramos cubiertos del cedido
#def comparar_ramos(row):
...
#Funcion Validación SubRamos: Valida si los subramos del tomado son disconjuntos de los subramos excluidos del cedido
#def comparar_subramos(row): 
...
# Función Validación Paises: Valida si los paises del tomado son disconjuntos de los paises excluidos del cedido
#def comparar_paises(row):
...
# Función Validación Fechas vigencia: Valida si la fecha de inicio de vigencia ocurre despues del inicio de la vigencia del contrato del cedido
#def comparar_vigencias(row):
...
#Función territorios: obtiene territorios del tomado de acuerdo a los paises de la consulta del tomado
#def Columna_territorio(paises):
    if not paises:  
        return ''
...
#Funcion validación Territorios
#def comparar_territorios(row):
    
    Territorios_cedidos = row['Territorios_Cubiertos'] if row['Territorios_Cubiertos'] is not None else ''
    Territorios_tomados = row['Territorio'] if row['Territorio'] is not None else ''

    Territorios_cedidos = set(Territorios_cedidos.split(', ')) if Territorios_cedidos else []
    Territorios_tomados = set(Territorios_tomados.split(', ')) if Territorios_tomados else []
        
    return set(Territorios_tomados) <= set(Territorios_cedidos) 
...
xFolder = r"C:\Users\asunad\OneDrive - GPV\Planeación Financiera RPAT - Reporting y Consultas\Consulta Identificación RetroEsp"
fileName_Retro = f"{xFolder}\\Ident_RetroEsp_Propv2.xlsm"
CedF_Ordenado.to_excel(fileName_Retro, index=False)
...
Ident_RetroEsp.save(f"{xFolder}\\Ident_RetroEsp_Propv2.xlsm")
